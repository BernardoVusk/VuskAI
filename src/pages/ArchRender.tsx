...
const ArchRender = () => {
  ...
  const [adminSubTab, setAdminSubTab] = useState<'keys' | 'users' | 'support'>('keys');
  const [isSupportModalOpen, setIsSupportModalOpen] = useState(false);
  const [mounted, setMounted] = useState(false);
  const [isSavoring, setIsSavoring] = useState(false);

  useEffect(() => {
    setMounted(true);
  }, []);

  const handleAnalyzeWithSavoring = async () => {
    setIsSavoring(true);
    // Deliberate delay for "Savoring Moment"
  ...
  return (
    <div className="relative flex min-h-screen bg-[#fafafa] text-zinc-900 selection:bg-indigo-500/10 overflow-x-hidden max-w-[100vw]">
      <CustomCursor />
      <div className="noise-overlay" />
      {/* Background Decorative Elements */}
      <div className="fixed inset-0 pointer-events-none overflow-hidden z-0">
        <div className="absolute top-[-10%] left-[-10%] w-[60%] h-[60%] bg-indigo-500/[0.03] blur-[120px] rounded-full" />
        <div className="absolute bottom-[-10%] right-[-10%] w-[60%] h-[60%] bg-indigo-500/[0.03] blur-[120px] rounded-full" />
                ...
                >
                  <item.icon 
                    ...
                    className={cn(
                      ...
                      archVizSubTab === item.id ? "text-white" : "text-zinc-400 group-hover:text-indigo-600"
                    )} 
                  />
      ...
      <main className={cn(
        ...
        mode === AnalysisMode.ARCHITECTURE ? "lg:ml-[520px]" : "lg:ml-[240px]"
      )}>
        ...
        <header className="mb-4 md:mb-16 flex flex-col md:flex-row items-start md:items-end justify-between gap-6 md:gap-8 relative z-10">
          ...
          >
            <h1 className="text-3xl sm:text-6xl md:text-fluid-8xl font-semibold tracking-tighter leading-[0.9] text-zinc-900">
              {mode === AnalysisMode.ARCHITECTURE && archVizSubTab !== 'ai' 
                ? archVizSubTab === 'prompts' ? 'Biblioteca de Prompts' : 'Aulas de Arquitetura'
                : mode === AnalysisMode.ARCHITECTURE ? 'Gerador de Arquitetura'
                : mode === AnalysisMode.IDENTITY ? 'Identidade Visual'
                : mode === AnalysisMode.LIFESTYLE ? 'Estilo de Vida'
                : mode === AnalysisMode.CINEMATIC ? 'Cinematográfico'
                : mode === AnalysisMode.MARKETPLACE ? 'Marketplace'
                : mode.split('_')[0]}
            </h1>
          </motion.div>
          ...
          >
            {currentSubscription ? (
              <div className="flex items-center gap-4 px-5 py-3 rounded-2xl bg-white border border-zinc-100 shadow-xl shadow-indigo-500/5 w-full md:w-auto justify-center md:justify-start">
                ...
                <div className="text-left">
                  <div className="text-[9px] md:text-[10px] font-bold text-indigo-600 uppercase tracking-tight leading-none mb-1">Acesso Pro Ativo</div>
                  <div className="text-[8px] md:text-[9px] font-medium text-zinc-400 uppercase tracking-tight leading-none">{calculateRemainingDays(currentSubscription.expiresAt)} dias restantes</div>
                </div>
              </div>
            ) : (
              <button 
                ...
                <div className="max-w-6xl mx-auto">
                  <div className="flex flex-col md:flex-row md:items-center justify-between gap-8 mb-12">
                    <div className="flex items-center gap-4">
                      ...
                      <div>
                        <h2 className="text-3xl font-semibold tracking-tighter text-zinc-900">Painel Administrativo</h2>
                        <p className="text-zinc-400 text-[10px] tracking-tight">Gerenciamento de sistema e usuários</p>
                      </div>
                    </div>
                    ...
                    <div className="flex bg-slate-100 p-1 rounded-xl">
                      <button 
                        ...
                        className={cn(
                          "px-6 py-2 rounded-lg text-[10px] font-semibold uppercase tracking-tight transition-all",
                          adminSubTab === 'keys' ? "bg-white text-black shadow-sm" : "text-zinc-500 hover:text-zinc-700"
                        )}
                      >
                        Gerador de Chaves
                      </button>
                      <button 
                        ...
                        className={cn(
                          "px-6 py-2 rounded-lg text-[10px] font-semibold uppercase tracking-tight transition-all",
                          adminSubTab === 'users' ? "bg-white text-black shadow-sm" : "text-zinc-500 hover:text-zinc-700"
                        )}
                      >
                        Gestão de Usuários
                      </button>
                      <button 
                        ...
                        className={cn(
                          "px-6 py-2 rounded-lg text-[10px] font-semibold uppercase tracking-tight transition-all",
                          adminSubTab === 'support' ? "bg-white text-black shadow-sm" : "text-zinc-500 hover:text-zinc-700"
                        )}
                      >
                        Suporte
                      </button>
                  ...
                  <GlassCard className="flex-1 flex flex-col p-0 relative group overflow-hidden border-x-0 md:border-x border-slate-200 bg-white md:rounded-[32px] rounded-3xl shadow-2xl">
                    ...
                    <div className="relative flex items-center justify-between p-4 md:p-6 border-b border-slate-100 z-20">
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          <div className="text-[10px] font-semibold uppercase tracking-tight text-zinc-900">Imagem de Referência</div>
                        </div>
                      </div>
                    </div>
                        ...
                        {image ? (
                          ...
                          >
                            <div className="relative mb-8">
                              <div className="absolute inset-0 bg-indigo-500/5 blur-2xl rounded-full scale-150 opacity-0 group-hover/upload:opacity-100 transition-opacity duration-500" />
                              <div className="relative w-24 h-24 rounded-3xl border border-zinc-200 bg-white flex items-center justify-center group-hover/upload:border-indigo-500/30 group-hover/upload:bg-indigo-500/5 transition-all duration-500 shadow-sm">
                                <Upload size={32} className="text-zinc-400 group-hover/upload:text-indigo-600 group-hover/upload:scale-110 transition-all" />
                              </div>
                            </div>
                    ...
                    <div className="p-4 md:p-6 pt-2">
                      <Button 
                        variant="custom"
                        className={cn(
                          "w-full h-12 md:h-16 rounded-2xl text-sm font-bold transition-all duration-500 active:scale-[0.98] tracking-tight",
                          !image || status === AppStatus.ANALYZING || isSavoring
                            ? "bg-zinc-100 text-zinc-400 border border-zinc-200" 
                            : "bg-indigo-600 text-white hover:bg-indigo-700 shadow-xl shadow-indigo-500/10"
                        )}
                        onClick={handleAnalyzeWithSavoring}
                        disabled={!image || status === AppStatus.ANALYZING || isSavoring}
                      >
                        {status === AppStatus.ANALYZING || isSavoring ? (
                          <div className="flex items-center gap-3">
                            <Loader2 size={18} className="animate-spin" />
                            <span>{isSavoring ? 'Analisando Materiais...' : 'Analisando...'}</span>
                          </div>
                        ) : (
                          <div className="flex items-center gap-3">
                            <Wand2 size={18} />
                  ...
                  <GlassCard className="flex-1 flex flex-col p-0 relative overflow-hidden border-slate-200 bg-white rounded-[32px] shadow-2xl">
                    ...
                    <div className="relative flex items-center justify-between p-4 sm:p-6 border-b border-slate-100 z-20">
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          <div className="text-xs font-semibold uppercase tracking-tight text-zinc-900">Resultado da IA</div>
                        </div>
                      </div>
                    ...
                    <div className="flex-1 p-4 md:p-8 relative overflow-y-auto custom-scrollbar">
                      <AnimatePresence mode="wait">
                        {error ? (
                          ...
                          >
                            <div className="relative mb-8">
                              <div className="w-24 h-24 border-2 border-indigo-100 rounded-full animate-spin border-t-indigo-600" />
                              <div className="absolute inset-0 flex items-center justify-center">
                                <Globe size={32} className="text-indigo-600/30 animate-pulse" />
                              </div>
                            ...
                            <div className="text-xs font-semibold uppercase tracking-tight text-zinc-400 animate-pulse">
                              {isSavoring ? 'Análise de Materiais...' : 'Processando camadas visuais...'}
                            </div>
                          ...
                          >
                            ...
                            <div className="text-xs font-semibold uppercase tracking-tight">Aguardando análise</div>
                          </motion.div>
                        ) : (
                          <motion.div 
                            key="results"
                              ...
                              <section className="space-y-6">
                                <div className="flex items-center justify-between">
                                  <div className="flex items-center gap-3">
                                    ...
                                    <h4 className="text-[10px] font-semibold uppercase tracking-tight text-zinc-900">Sugestão de Prompt</h4>
                                  </div>
                                  <button 
                                    onClick={handleCopyPrompt}
                                    className="flex items-center gap-2 px-4 py-2 rounded-xl bg-zinc-50 border border-zinc-200 text-[10px] font-semibold uppercase tracking-tight text-zinc-600 hover:bg-zinc-100 hover:text-black transition-all active:scale-95 shadow-sm"
                                  >
                                ...
                                <div className="p-6 rounded-[32px] bg-slate-50/50 border border-slate-200/60 backdrop-blur-sm">
                                  <div className="flex items-center gap-4 mb-6">
                                    ...
                                    <div>
                                      <div className="text-[10px] font-semibold uppercase tracking-tight text-zinc-900 leading-none mb-1">Presets Cinematográficos</div>
                                      <div className="text-[9px] font-medium uppercase tracking-tight text-zinc-400 leading-none">Otimizado para Veo & Sora</div>
                                    </div>
                                  </div>
                                  ...
                                  <div className="space-y-4">
                                    <div className="relative group/select">
                                      <select 
                                        ...
                                        className="w-full bg-white border border-zinc-200 rounded-2xl px-5 py-4 text-sm text-zinc-900 appearance-none focus:outline-none focus:border-indigo-600 focus:ring-4 focus:ring-indigo-600/5 transition-all cursor-pointer font-semibold shadow-sm"
                                      >
                                      ...
                                      <div className="absolute right-5 top-1/2 -translate-y-1/2 pointer-events-none text-zinc-400 group-hover/select:text-indigo-600 transition-colors">
                                        <ChevronDown size={18} />
                                      </div>
                                    </div>
                                        ...
                                        >
                                          <div className="pt-2 space-y-4">
                                            ...
                                            <Button 
                                              ...
                                              className="w-full bg-indigo-600 text-white hover:bg-indigo-700 rounded-2xl py-5 flex items-center justify-center gap-3 font-semibold text-[10px] uppercase tracking-tight shadow-xl shadow-indigo-500/10 transition-all hover:-translate-y-1 active:scale-95"
                                            >
                                ...
                                <div className="p-6 rounded-[32px] bg-slate-50/50 border border-slate-200/60 backdrop-blur-sm">
                                  <div className="flex items-center gap-4 mb-6">
                                    ...
                                    <div>
                                      <div className="text-[10px] font-semibold uppercase tracking-tight text-zinc-900 leading-none mb-1">Clima e Horário</div>
                                      <div className="text-[9px] font-medium uppercase tracking-tight text-zinc-400 leading-none">Ajuste a atmosfera da cena</div>
                                    </div>
                                  </div>
                                  ...
                                  <div className="grid grid-cols-2 sm:grid-cols-5 gap-2">
                                    {weatherPresets.map((preset) => (
                                      <button
                                        ...
                                        disabled={isRefining}
                                        className="flex flex-col items-center justify-center gap-2 p-3 rounded-2xl bg-white border border-zinc-200/50 hover:border-indigo-500/30 hover:bg-indigo-50/50 transition-all group active:scale-95 disabled:opacity-50 disabled:cursor-not-allowed shadow-sm"
                                      >
                                        <preset.icon size={18} className="text-zinc-400 group-hover:text-indigo-600 transition-colors" />
                                        <span className="text-[9px] font-semibold uppercase tracking-tight text-zinc-400 group-hover:text-zinc-900 transition-colors text-center">{preset.name}</span>
                                      </button>
                                    ))}
                                  </div>
                                </div>
                                ...
                                {/* Refinement UI */}
                                <div className="p-6 rounded-[32px] bg-indigo-50/30 border border-indigo-100/50">
                                  <div className="text-[10px] font-semibold uppercase tracking-tight text-indigo-900 mb-4">Refinar Análise</div>
                                  <div className="flex gap-3">
                                    <div className="relative flex-1">
                                      <input
                                        ...
                                        className="w-full bg-white border border-indigo-100 rounded-2xl px-5 py-4 text-sm text-zinc-900 placeholder-zinc-400 focus:outline-none focus:border-indigo-600 focus:ring-4 focus:ring-indigo-600/5 transition-all font-semibold shadow-sm"
                                        disabled={isRefining}
                                      />
                                    </div>
                                    <Button 
                                      ...
                                      className="bg-indigo-600 text-white hover:bg-indigo-700 rounded-2xl px-6 shadow-lg shadow-indigo-500/20 active:scale-95 transition-all"
                                    >
                            ...
                            {resultImage && (
                              <section className="space-y-6">
                                <div className="flex items-center gap-3">
                                  ...
                                  <h4 className="text-[10px] font-semibold uppercase tracking-tight text-zinc-900">Resultado Visual</h4>
                                </div>
                                <div className="relative group/asset rounded-[32px] overflow-hidden border border-slate-200 shadow-2xl bg-white p-2">
                                  <div className="relative rounded-[24px] overflow-hidden">
                                    ...
                                    <div className="absolute inset-0 bg-gradient-to-t from-black/80 via-transparent to-transparent opacity-0 group-hover/asset:opacity-100 transition-all duration-500 flex items-end justify-between p-8">
                                      <Button size="sm" variant="outline" className="bg-white/10 backdrop-blur-md border-white/20 text-white rounded-xl hover:bg-white/20 font-semibold text-[10px] uppercase tracking-tight">
                                        <Maximize2 size={14} className="mr-2" /> Expandir
                                      </Button>
                                      <Button size="sm" className="bg-white text-black font-semibold rounded-xl px-8 py-4 hover:bg-zinc-100 transition-all active:scale-95 text-[10px] uppercase tracking-tight shadow-xl">
                                        Download
                                      </Button>
                                    </div>
                                  </div>
                    ...
                    {mode !== AnalysisMode.ARCHITECTURE && analysis && !resultImage && (
                      <div className="p-6 border-t border-slate-100 bg-slate-50">
                        <Button 
                          variant="custom"
                          className={cn(
                            "w-full h-16 rounded-2xl text-sm font-semibold transition-all duration-500 uppercase tracking-tight",
      ...
      <AnimatePresence>
        {!currentSubscription && mode !== AnalysisMode.ADMIN_KEYS && mode !== AnalysisMode.SETTINGS && (
          ...
          >
            ...
            <h3 className="text-3xl font-semibold tracking-tighter uppercase mb-4 text-zinc-900">Módulo Bloqueado</h3>

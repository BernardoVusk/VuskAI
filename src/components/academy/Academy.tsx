  ...
  if (loading) {
    return (
      <div className="flex flex-col items-center justify-center h-64">
        ...
        <p className="text-slate-500 text-xs font-medium">Carregando Aulas...</p>
      </div>
    );
  }
    ...
    <div className="w-full max-w-6xl mx-auto space-y-8 pb-20">
      ...
      {isAdmin && (
        <div className="flex gap-4 mb-8">
          <Button 
            ...
            className="bg-white/5 border border-white/10 text-white hover:bg-white/10 rounded-xl px-6 py-2 flex items-center gap-2"
          >
            <Plus size={16} />
            <span className="text-xs font-semibold">Novo Módulo</span>
          </Button>
          <Button 
            onClick={() => {
              setEditingLesson(null);
            ...
            className="bg-white text-black hover:bg-slate-200 rounded-xl px-6 py-2 flex items-center gap-2"
          >
            <Video size={16} />
            <span className="text-xs font-semibold">Nova Aula</span>
          </Button>
        </div>
      )}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
              ...
              >
                <div className="flex items-center gap-3">
                  ...
                  <div className="text-left">
                    ...
                    <p className="text-[10px] text-slate-500 truncate max-w-[200px]">{mod.description}</p>
                  </div>
                </div>
                  ...
                  {isAdmin && (
                    <div className="flex gap-1 mr-2" onClick={(e) => e.stopPropagation()}>
                      <button 
                        ...
                        className="p-2.5 hover:text-white hover:bg-white/10 rounded-lg transition-all"
                        title="Editar Módulo"
                      >
                        <Edit size={16} />
                      </button>
                      <button 
                        ...
                        className="p-2.5 hover:text-red-400 hover:bg-red-500/10 rounded-lg transition-all"
                      ...
                      >
                        <div className="flex items-center gap-3">
                          <div className={cn(
                            ...
                            selectedLesson?.id === lesson.id ? "bg-emerald-500/20" : "bg-white/5 group-hover:bg-white/10"
                          )}>
                        ...
                        {isAdmin && (
                          <div className="flex gap-1 opacity-0 group-hover:opacity-100 transition-opacity" onClick={(e) => e.stopPropagation()}>
                            <button 
                              ...
                              className="p-2 hover:text-white hover:bg-white/10 rounded-lg transition-all"
                              title="Editar Aula"
                            >
                              <Edit size={12} />
                            </button>
        ...
        <div className="lg:col-span-7">
          <GlassCard className="aspect-video relative overflow-hidden border-white/5 bg-black/40 backdrop-blur-3xl rounded-[32px] shadow-2xl flex items-center justify-center">
            {selectedLesson ? (
              <iframe
              ...
              <div className="flex flex-col items-center text-slate-700">
                ...
                <p className="text-xs font-medium">Selecione uma aula para começar</p>
              </div>
            )}
          </GlassCard>
          ...
          {selectedLesson && (
            <motion.div
              ...
              className="mt-6 p-6 rounded-[24px] bg-white/5 border border-white/5"
            >
              <h2 className="text-xl font-bold text-white mb-2 uppercase tracking-tight">{selectedLesson.title}</h2>
              <p className="text-sm text-slate-400 leading-relaxed">{selectedLesson.description}</p>
            </motion.div>
          )}
        </div>
      </div>
          ...
          <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-black/80 backdrop-blur-sm">
            ...
            >
              <div className="flex items-center justify-between mb-8">
                ...
                <button onClick={() => setIsModuleModalOpen(false)} className="text-slate-500 hover:text-white">
                  <X size={20} />
                </button>
              </div>
              ...
              <div className="space-y-6">
                <div>
                  <label className="block text-xs font-semibold text-slate-400 mb-2">Título</label>
                  <input
                    type="text"
                ...
                <div>
                  ...
                  <textarea
                    ...
                    className="w-full bg-black/40 border border-white/10 rounded-xl px-4 py-3 text-sm text-white focus:outline-none focus:border-white/30 h-24 resize-none"
                  />
                </div>
                <div>
                  <label className="block text-xs font-semibold text-slate-400 mb-2">Ordem</label>
                  <input
                    type="number"
          ...
          <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-black/80 backdrop-blur-sm">
            ...
            >
              <div className="flex items-center justify-between mb-8">
                ...
                <button onClick={() => setIsLessonModalOpen(false)} className="text-slate-500 hover:text-white">
                  <X size={20} />
                </button>
              </div>
              ...
              <div className="space-y-6">
                <div>
                  <label className="block text-xs font-semibold text-slate-400 mb-2">Módulo</label>
                  <select
                ...
                <div>
                  <label className="block text-xs font-semibold text-slate-400 mb-2">ID do Vídeo (YouTube)</label>
                  <input
                    type="text"
        ...
        {itemToDelete && (
          <div className="fixed inset-0 z-[110] flex items-center justify-center p-4 bg-black/90 backdrop-blur-md">
            <motion.div
              ...
              className="w-full max-w-sm bg-zinc-900 border border-red-500/20 rounded-[32px] p-8 shadow-2xl text-center"
            >
              ...
              <div className="grid grid-cols-2 gap-4">
                <button
                  ...
                  className="px-6 py-3 rounded-xl bg-white/5 text-slate-300 font-bold text-xs hover:bg-white/10 transition-colors"
                >
                  Cancelar
                </button>
                <button
                  ...
                  disabled={loading}
                  className="px-6 py-3 rounded-xl bg-red-600 text-white font-bold text-xs hover:bg-red-500 transition-colors disabled:opacity-50 flex items-center justify-center gap-2"
                >
